...
def main(piIsAlive, tryAgain, theta, dist, vs, timerstart, count, display, screenText, rightCenter, leftCenter, turnangle, arb, ars):
    piAlive()
...
##                print(FPS)
        ...
        vs.stop()
import threading
# create a thread object that will do the main alphabot code in the background
class counter(threading.Thread):
    ...
    def run(self): # this is the main function that will run in a separate thread
        self.alive = True
        while self.alive:
            main(piIsAlive, tryAgain, theta, dist, vs, timerstart, count, display, screenText, rightCenter, leftCenter, turnangle, arb, ars)
            network(s)
            nt.initialize(server=ip)
            sd = nt.getTable("SmartDashboard")
...
def Main():
    a = main(piIsAlive, tryAgain, theta, dist, vs, timerstart, count, display, screenText, rightCenter, leftCenter, turnangle, arb, ars)
    b = network(s)
    a.start()
    b.start()
    nt.initialize(server=ip)
    sd = nt.getTable("SmartDashboard")
    while True:
        Input = input("Do you want network tables (y/n): ")
        if Input == "y":
            network(s)
            ip,connected = network(s)
            print('Everything is Connected Starting Camera and Vision')
            nt.initialize(server=ip)
            sd = nt.getTable("SmartDashboard")
            main(piIsAlive, tryAgain, theta, dist, vs, timerstart, count, display, screenText, rightCenter, leftCenter, turnangle, arb, ars)
        elif Input == "n":
