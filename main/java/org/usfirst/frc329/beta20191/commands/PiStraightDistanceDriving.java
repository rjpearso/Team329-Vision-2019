...
public class PiStraightDistanceDriving extends Command {
  ...
  private static final double MIN_POWER = .3;  //.2
  ...
  private static final double ALMOST_ZERO = .5;
  private static final double ALMOST_THERE = 8;
  ...
  private static final double DONE_COUNT = 10;
  private static int counter;

  public PiStraightDistanceDriving() {
  ...
  protected void initialize() {
    d = SmartDashboard.getNumber("Distance Away", d) - 8; //-8 because we don't want to crash
    a = SmartDashboard.getNumber("Turn Angle", a);
    counter = 0;
    Robot.sDrive.piSDDOkay = true;
    if(a <= BAD_ANGLE || a >= Math.abs(BAD_ANGLE) || Robot.sPi.piRunning() == false){ // cant see the target    || turnAng >= Math.abs(BAD_ANGLE)
      System.out.println("THE ANGLE IS BAD PSDD");
      d = 0; a = 0;
      Robot.sPi.setSeeAngle(false);
    } 
    else Robot.sPi.setSeeAngle(true);
    a = a + Robot.sDrive.getYaw();
    initDistDrive();
    initStraightDrive();
  }
  ...
  protected boolean isFinished() {
    if((Math.abs(d - currLoc) < GOOD_ENOUGH)&&(currabsVel < MIN_VEL)){
      Robot.sDrive.piSDDOkay = false;
      return true;
    } 
    else if((Math.abs(d - currLoc) < ALMOST_THERE)&&(currabsVel < ALMOST_ZERO)){
      counter++;
  ...
  protected void end() {
    ...
    System.out.println("PiStraightDistanceDriving Done" + "  currLoc = " + currLoc);
  }
